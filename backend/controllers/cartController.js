...
const getCart = async (req, res) => {
	try {
		const user = await User.findOne({ email: req.user.email }).populate({
			path: 'cart',
			populate: { path: 'products', model: 'Product' }  // nested populate
		});
...
const addToCart = async (req, res) => {
	try {
		...
		if (user.cart) {
			...
			// First time — create a new cart and link it to the user
		...
		// Recalculate total from actual product prices
...
const removeFromCart = async (req, res) => {
	try {
		...
		cart.products.splice(index, 1);  // remove 1 item at that index
		cart.total = cart.products.reduce((sum, p) => sum + p.price, 0);
		await cart.save();
